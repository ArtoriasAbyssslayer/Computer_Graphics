...
class TriangleFillingFunction:
	def __init__(self):
		pass
	def interpolate_vector_color(self,x1,x2,C1,C2):
		# '''
		...
		# '''
		if x1 == x2:
			val = V1 * np.ones(x1.shape[1],3)
			return val

		if x1[0]==x2[0]:
			# interpolate according y_coordinate
			gamma = abs(x2[1]-x[1])/abs(x2[1]-x1[1])
			interpolation_value = np.array(gamma*C1 + (1-gamma)*C2)
		elif x1[1]==x2[1]:
			...
			interpolation_value = np.mean(interpolation_arr, axis=0)
