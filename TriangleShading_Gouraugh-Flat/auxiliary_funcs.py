...
def render_image_save_file(img,save=False,filename)
	if save == True:
		cv.imshow("Generated image - Fish",img)
		cv.waitKey('0')
		np.save(open(filename+".npy",wb+),img)
		# or this
		pickle.dump(img,open(filename+"_pickle.npy","wb+"))
		read_image = pickle.laod(open(filename+"_pickle.npy","rb"))
...
def find_edge_bounds(verts2d):
	...
	x_bounds = np.array([np.ndarray.min(edge_vertices[:,:,0],axis=1), np.ndarray.max(edge_vertices[:,:,0],axis=1)])
	y_bounds = np.array([np.ndarray.min(edge_vertices[:,:,1],axis=1), np.ndarray.max(edge_vertices[:,:,1],axis=1)])

    edge_line_alpha = (y_bounds[1]- edge_vertices[:,:,1])/(x_bounds[1]-edge_vertices[:,:,0])
...
def predetermined_active(active_edges, active_nodes, vertices, y_bounds, edge_lines_alphas):
	...
	for i,y_bound in enumerate(y_bounds):
		if y_bound[0] == y_min:
			if alpha == 0:
				continue #its the lower horizontal line(for other lines like this inside we deal later)
			if np.isnan(edge_lines_alphas[i]): # alpha calculation is based on 2d so if it is nan we have an edge that is not projected
				no_projected = True
				continue
			active_edges[i] = True
