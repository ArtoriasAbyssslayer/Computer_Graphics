...
import common_utils.render as render
    ...
    for i, ybound in enumerate(active_ybounds):
        if ybound[0] == y_min:  # y-scanline meets new edge from bottom
            if edge_alpha[i] == 0:  # vertical line
                continue
            if np.isnan(edge_alpha).all():  # its a line with no slope aka invisible
                is_invisible = True
                continue
...
def updateActiveEdges(active_edges, edge_verts, y, edge_alpha, y_bounds):
    ...
    return np.array(active_edges), np.array(updated_active_bounds), np.array(updated_points)
