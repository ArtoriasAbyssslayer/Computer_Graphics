...
def rasterize(p2d,Rows,Columns,H,W):
    ...
    for i in range(num_points):
        RastCanvas[i][0] = np.around((p2d[i][0]+H/2)*vertical_ratio-0.5)
        RastCanvas[i][1] = np.around((p2d[i][1]+W/2)*horizontal_ratio-0.5)
        
    # Roatate the image by 90 degrees
    # y = -y 
    # x = -x
    # To make the rendered object pose upward
